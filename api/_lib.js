...
function ema(values, period){
  // values: newest first. Returns array newest-first of EMA.
  const k = 2/(period+1);
  const rev = values.slice().reverse(); // oldest first
  const out = [];
  let prev;
  rev.forEach((v,i)=>{
    if(i===0){ prev = v; }
...
function rsi(closesNewestFirst, period=14){
  ...
  const closes = closesNewestFirst.slice(0, period+1).reverse(); // oldest->newest, period+1 points
...
function macd(closesNewestFirst){
  ...
  const macdLine = ema12.map((v,i)=> v-ema26[i]); // newest first, same length as ema26
